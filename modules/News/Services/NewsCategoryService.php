...
class NewsCategoryService
{
    ...
    public function all(): array
    {
        try {
            return $this->db->fetchAll(
                "SELECT * FROM news_categories ORDER BY position ASC, id ASC"
            );
        } catch (\Throwable $e) {
            return [];
        }
    ...
    {
        $row = $this->db->fetch("SELECT * FROM news_categories WHERE id = ?", [$id]);
        return $row ?: null;
    }
    ...
    public function create(array $data): int
    {
        ...
        $this->db->execute(
            "INSERT INTO news_categories (slug, name_en, name_ru, image_url, position, enabled)
             VALUES (?, ?, ?, ?, ?, ?)",
            [
                $slug,
                $data['name_en'] ?? '',
