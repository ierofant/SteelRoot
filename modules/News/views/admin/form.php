...
$buildUrl = static function(array $extra) use ($ap, $categoryFilterId, $authorFilterId, $queryFilter): string {
    ...
    return htmlspecialchars($ap . '/news' . (!empty($query) ? ('?' . http_build_query($query)) : ''));
};
    ...
    <div class="card">
        ...
        <div class="card-header">
            ...
            <div class="u-flex u-gap-half">
                <a class="btn ghost" href="<?= htmlspecialchars($ap) ?>/news/settings">Настройки</a>
                <a class="btn ghost" href="<?= htmlspecialchars($ap) ?>/news/categories">Управление категориями</a>
                <a class="btn ghost" href="<?= htmlspecialchars($ap) ?>/news/categories/create">+ Создать категорию</a>
                <a class="btn primary" href="<?= htmlspecialchars($ap) ?>/news/create?return=<?= rawurlencode($listReturn) ?>">+ Создать новость</a>
            </div>
        </div>
        <form method="get" action="<?= htmlspecialchars($ap) ?>/news" class="stack">
            ...
            <div class="form-actions">
                ...
                <a class="btn ghost" href="<?= htmlspecialchars($ap) ?>/news"><?= __('articles.filter.reset') ?></a>
            </div>
        </form>
                ...
                <tbody>
                    <?php foreach ($articles as $a): ?>
                        ...
                        <tr>
                            ...
                            <td class="actions">
                                <a class="btn ghost small" href="<?= htmlspecialchars($ap) ?>/news/edit/<?= urlencode($a['slug']) ?>?return=<?= rawurlencode($listReturn) ?>"><?= __('articles.action.edit') ?></a>
                                <form method="post" action="<?= htmlspecialchars($ap) ?>/news/delete/<?= urlencode($a['slug']) ?>" onsubmit="return confirm('<?= __('articles.action.confirm_delete') ?>');">
...
<?php else: ?>
    ...
    <div class="card stack">
            <div class="card-header">
                <div>
            <p class="eyebrow"><?= $isEdit ? 'Редактирование новости' : 'Новая новость' ?></p>
            <h3><?= $isEdit ? 'Редактировать новость' : 'Создать новость' ?></h3>
                </div>
            <a class="btn ghost" href="<?= htmlspecialchars($ap) ?>/news"><?= __('articles.action.back') ?></a>
        </div>
            <form method="post" action="<?= $isEdit ? $ap . '/news/edit/' . urlencode($article['slug'] ?? '') : $ap . '/news/create' ?>" class="stack article-editor-form" enctype="multipart/form-data" id="article-form">
                <input type="hidden" name="_token" value="<?= htmlspecialchars($csrf ?? '') ?>">
            ...
            <label class="field">
                ...
                <?php if (empty($categories)): ?>
                    <span class="muted u-font-085em">
                        Категорий пока нет. <a href="<?= htmlspecialchars($ap) ?>/news/categories/create">Создать категорию</a>
                    </span>
                <?php endif; ?>
            </label>
            <?php if (!empty($users)): ?>
    ...
    <script>
        ...
        const editorPrefKey = 'news_editor_enabled';
        let mdeEn = null;
        let mdeRu = null;
